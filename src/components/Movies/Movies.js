...
function Movies() {
  ...
  useEffect(() => {
    ...
    if (searchMovie) { loadFilmes(searchMovie, isToggleMovie); }
  }, []);
  ...
  const loadFilmes = (value, toggle) => {
    ...
    getMoviesApi(value)
      .then((response) => {
        let cardsMovies = response.filter((x) => x.nameRU.toLowerCase().includes(value.toLowerCase())
          || x.nameEN.toLowerCase().includes(value.toLowerCase()));
        if (toggle) { cardsMovies = cardsMovies.filter((x) => x.duration < 60); }
  ...
  const handleSearch = (value) => {
    ...
    if (!storageData) {
      const newData = { ...initStorageData };
      localStorage.setItem('storageData', JSON.stringify({ ...newData, searchMovie: value }));
    } else {
      const newData = { ...JSON.parse(storageData), searchMovie: value };
  ...
  const messageNotFound = () => <p>Ничего не найдено...</p>;
  ...
  const handleToggle = (value) => {
    ...
    if (!storageData) {
      ...
      const newData = { ...JSON.parse(storageData), isToggleMovie: value };
      localStorage.setItem('storageData', JSON.stringify(newData));
    }
